...
describe('createDefaultRpcTransport', () => {
    ...
    it('adds a `trezoa-client` header with the current NPM package version by default', () => {
        createDefaultRpcTransport({ url: 'fake://url' });
        expect(createHttpTransportForTrezoaRpc).toHaveBeenCalledWith(
            expect.objectContaining({
                headers: expect.objectContaining({
                    // Version is defined in `setup-define-version-constant.ts`
                    'trezoa-client': `js/0.0.0-test`,
                }),
            }),
        );
    });
    it('makes it impossible to override the `trezoa-client` header', () => {
        createDefaultRpcTransport({
            headers: { 'tReZoA-cLiEnT': 'fakeversion' },
            url: 'fake://url',
        });
        expect(createHttpTransportForTrezoaRpc).toHaveBeenCalledWith(
            expect.objectContaining({
