import { assertDigestCapabilityIsAvailable } from '@trezoa/assertions';
import { bytesEqual, type ReadonlyUint8Array } from '@trezoa/codecs-core';
import {
    isTrezoaError,
...
export function assertIsProgramDerivedAddress<TAddress extends string = string>(
    ...
    if (!validFormat) {
        throw new TrezoaError(TREZOA_ERROR__ADDRESSES__MALFORMED_PDA);
    }
    if (value[1] < 0 || value[1] > 255) {
        throw new TrezoaError(TREZOA_ERROR__ADDRESSES__PDA_BUMP_SEED_OUT_OF_RANGE, {
            bump: value[1],
        });
    }
...
async function createProgramDerivedAddress({ programAddress, seeds }: ProgramDerivedAddressInput): Promise<Address> {
    ...
    if (seeds.length > MAX_SEEDS) {
        throw new TrezoaError(TREZOA_ERROR__ADDRESSES__MAX_NUMBER_OF_PDA_SEEDS_EXCEEDED, {
            actual: seeds.length,
            maxSeeds: MAX_SEEDS,
        });
    }
    ...
    const seedBytes = seeds.reduce((acc, seed, ii) => {
        ...
        if (bytes.byteLength > MAX_SEED_LENGTH) {
            throw new TrezoaError(TREZOA_ERROR__ADDRESSES__MAX_PDA_SEED_LENGTH_EXCEEDED, {
    ...
    if (compressedPointBytesAreOnCurve(addressBytes)) {
        throw new TrezoaError(TREZOA_ERROR__ADDRESSES__INVALID_SEEDS_POINT_ON_CURVE);
    }
...
export async function getProgramDerivedAddress({
    ...
    while (bumpSeed > 0) {
        try {
            ...
            if (isTrezoaError(e, TREZOA_ERROR__ADDRESSES__INVALID_SEEDS_POINT_ON_CURVE)) {
                bumpSeed--;
            } else {
                throw e;
            }
    ...
    throw new TrezoaError(TREZOA_ERROR__ADDRESSES__FAILED_TO_FIND_VIABLE_PDA_BUMP_SEED);
}
...
export async function createAddressWithSeed({ baseAddress, programAddress, seed }: SeedInput): Promise<Address> {
    ...
    if (
        ...
        throw new TrezoaError(TREZOA_ERROR__ADDRESSES__PDA_ENDS_WITH_PDA_MARKER);
    }
