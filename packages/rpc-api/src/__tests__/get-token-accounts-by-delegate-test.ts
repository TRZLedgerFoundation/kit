...
describe('getTokenAccountsByDelegate', () => {
    ...
    (['confirmed', 'finalized', 'processed'] as Commitment[]).forEach(commitment => {
        describe(`when called with \`${commitment}\` commitment`, () => {
            it('returns RPC response with account info', async () => {
                expect.assertions(1);
                // Delegate for fixtures/tpl-token-token-account-delegated.json
                const delegate =
    ...
    describe('when called with a mint that has a delegated token account', () => {
        it('returns RPC response with account info', async () => {
            ...
            // Mint at fixtures/tpl-token-mint-account-with-delegated.json (mint for above delegated account)
            const mint =
    ...
    describe('when called with a mint that has no delegated token accounts', () => {
        it('returns an empty list', async () => {
            ...
            // Mint at fixtures/tpl-token-mint-no-token-accounts.json (not the same mint as above delegated account)
            const mint =
