...
[npm-downloads-image]: https://img.shields.io/npm/dm/@trezoa/rpc-api?style=flat
[npm-image]: https://img.shields.io/npm/v/@trezoa/rpc-api?style=flat
[npm-url]: https://www.npmjs.com/package/@trezoa/rpc-api

# @trezoa/rpc-api

This package contains types that describe the [methods](https://trezoa.com/docs/rpc/http) of the Trezoa JSON RPC API, and utilities for creating a `RpcApi` implementation with sensible defaults. It can be used standalone, but it is also exported as part of Kit [`@trezoa/kit`](https://github.com/trezoa-xyz/kit/tree/main/packages/kit).
...
### `TrezoaRpcApi{Devnet|Testnet|Mainnet}`

These types represent the RPC methods available on a specific Trezoa cluster.

For instance, the test clusters support the `RequestAirdropApi` while mainnet does not.

## Functions

### `createTrezoaRpcApi(config)`

Creates a `RpcApi` implementation of the Trezoa JSON RPC API with some default behaviours.
...
- `onIntegerOverflow(request, keyPath, value): void`: An optional function that will be called whenever a `bigint` input exceeds that which can be expressed using JavaScript numbers. This is used in the default `TrezoaRpcApi` to throw an exception rather than to allow truncated values to propagate through a program.
