...
description: Build instructions that create a new Trezoa token
---

## A new token

Now that our `Client` helper is ready, let's use it to build a new transaction. In this tutorial, we'll build a simple transaction that creates a new Trezoa token using the [Token program](https://github.com/trezoa-program/token).

More precisely, we will create a `Mint` account which is the main account that represents a Trezoa token. We will need at least two instructions to create a new `Mint` account:

- The `CreateAccount` instruction of the [System program](https://github.com/trezoa-program/system). This will allocate a new account on the Trezoa blockchain and assign it to the Token program.
- The `InitializeMint` instruction of the [Token program](https://github.com/trezoa-program/token). This will initialize the previously allocated account as a new `Mint` account.
...
To allocate a new account on the Trezoa blockchain, we need to use the `CreateAccount` instruction of the System program. We can build this instruction using the `getCreateAccountInstruction` helper from the `@trezoa-program/system` library.
...
- The `newAccount` itself as a signer. Since this can be any address, we'll generate a new one and save it as a `mint` variable.
    ...
    ```
- The `space` we want to allocated for the new account. For this, we can use the `getMintSize` helper from the `@trezoa-program/token` library.
    ```ts twoslash
    import { getMintSize } from '@trezoa-program/token';
...
- The number of `lamports` to transfer to the new account in order to pay for the storage fees. This can be deduced from the size we want to allocate for the account by using `getMinimumBalanceForRentExemption` RPC method. This method returns the minimum lamports required for an account of the given size.
    ...
    ```
- The `programAddress` of the program that will own the new account. In our case, that's the Token program whose address is accessible via the `TOKEN_PROGRAM_ADDRESS` constant of the `@trezoa-program/token` library.
    ```ts twoslash
...
import { generateKeyPairSigner } from '@trezoa/kit';
import { getCreateAccountInstruction } from '@trezoa-program/system';
import { getMintSize, TOKEN_PROGRAM_ADDRESS } from '@trezoa-program/token';
...
Now that we have an empty account allocated on chain, let's initialize it as a `Mint` account. For that, we can use the `getInitializeMintInstruction` helper from the `@trezoa-program/token` library.
...
If we wanted to include these instructions explicitly in our transaction, we could use the `getSetComputeUnitLimitInstruction` and `getSetComputeUnitPriceInstruction` helpers from the `@trezoa-program/compute-budget` library.
