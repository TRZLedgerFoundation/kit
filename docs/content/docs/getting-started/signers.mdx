...
```tsx twoslash
import { generateKeyPairSigner, KeyPairSigner, TransactionSigner } from '@trezoa/kit';
...
// Generate a new wallet with 1 TRZ.
const wallet = await generateKeyPairSigner();
await airdrop({
...
We've now got everything we need to start crafting transactions! In the next article, we'll build instructions that create a new token on Trezoa.
