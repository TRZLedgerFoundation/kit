...
In the previous article of this series, we've learned how to send a transaction that creates a new Trezoa token — also known as a `Mint` account. In this last article, we'll learn how to retrieve the account we created and access its decoded data.
...
```ts twoslash
import { Rpc, TrezoaRpcApi } from '@trezoa/kit';
...
Fortunately for us, we often don't need to manually create these Codec objects as program clients will provide them for us. These helpers are typically [generated via Codoma IDLs](https://github.com/codoma-idl/codoma) and follow the `getXCodec` naming convention where `X` is the name of the account.

For instance, here's how we can access the `Mint` codec from the `@trezoa-program/token` library.
...
import { getMintCodec, Mint } from '@trezoa-program/token';
...
const rpc = null as unknown as Rpc<TrezoaRpcApi>;
// ---cut-before---
import { Account, address, assertAccountExists, fetchEncodedAccount } from '@trezoa/kit';
import { decodeMint, Mint } from '@trezoa-program/token';
...
import { fetchMint, Mint } from '@trezoa-program/token';
...
// @filename: index.ts
// ---cut-before---
import { unwrapOption } from '@trezoa/kit'; // [!code ++]
import { fetchMint } from '@trezoa-program/token'; // [!code ++]
