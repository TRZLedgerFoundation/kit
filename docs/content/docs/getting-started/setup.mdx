...
First things first, set up a new TypeScript project with a `src/index.ts` file and install the `@trezoa/kit` library. Additionally, to keep this tutorial focused on the Kit library, we'll use `tsx` to compile and run our TypeScript code.
