...
```ts twoslash
import { Instruction, sequentialInstructionPlan, parallelInstructionPlan } from '@trezoa/kit';
...
```ts twoslash
import { TransactionPlanner, TransactionPlanExecutor, InstructionPlan } from '@trezoa/kit';
...
Instruction plans are **included within the `@trezoa/kit` library** but you may also install them using their standalone package.
...
```ts twoslash
import { singleInstructionPlan, Instruction } from '@trezoa/kit';
const transferTrz = {} as unknown as Instruction;
// ---cut-before---
const instructionPlan = singleInstructionPlan(transferTrz);
```
...
```ts twoslash
import { singleInstructionPlan, sequentialInstructionPlan, Instruction } from '@trezoa/kit';
...
```ts twoslash
import { sequentialInstructionPlan, Instruction } from '@trezoa/kit';
const transferFromAliceToBob = {} as unknown as Instruction;
const transferFromBobToCarla = {} as unknown as Instruction;
// ---cut-before---
...
```ts twoslash
import { nonDivisibleSequentialInstructionPlan, Instruction } from '@trezoa/kit';
...
```ts twoslash
import { singleInstructionPlan, parallelInstructionPlan, Instruction } from '@trezoa/kit';
const transferFromAliceToBob = {} as unknown as Instruction;
...
```ts twoslash
import { parallelInstructionPlan, Instruction } from '@trezoa/kit';
const transferFromAliceToBob = {} as unknown as Instruction;
const transferFromAliceToCarla = {} as unknown as Instruction;
// ---cut-before---
...
```ts twoslash
import { getLinearMessagePackerInstructionPlan, Instruction } from '@trezoa/kit';
...
```ts twoslash
import { getReallocMessagePackerInstructionPlan, Instruction } from '@trezoa/kit';
...
```ts twoslash
import { BaseTransactionMessage, TransactionMessageWithFeePayer } from '@trezoa/kit';
// ---cut-before---
...
```ts twoslash
import { TransactionPlanner, InstructionPlan } from '@trezoa/kit';
const transactionPlanner = {} as unknown as TransactionPlanner;
const instructionPlan = {} as unknown as InstructionPlan;
...
In the example below, we check if the packed transaction contains an instruction that transfers TRZ and, if so, add a guard instruction that ensures no more than 1 TRZ is transferred.
...
const transferTrzGuardInstruction = {} as unknown as Instruction;
// ---cut-before---
const transactionPlanner = createTransactionPlanner({
    createTransactionMessage,
...
```ts twoslash
import { InstructionPlan, TransactionPlan } from '@trezoa/kit';
// ---cut-before---
...
```ts twoslash
import { TransactionPlanExecutor, TransactionPlan } from '@trezoa/kit';
const transactionPlanExecutor = {} as unknown as TransactionPlanExecutor;
...
const rpc = {} as unknown as Rpc<TrezoaRpcApi>;
const rpcSubscriptions = {} as unknown as RpcSubscriptions<TrezoaRpcSubscriptionsApi>;
// ---cut-before---
...
const error = {} as unknown as TrezoaError;
// ---cut-before---
...
When a transaction plan executor — created via the `createTransactionPlanExecutor` helper — encounters a failed transaction, it will cancel all remaining transactions in the plan. The executor will then throw a `TREZOA_ERROR__INSTRUCTION_PLANS__FAILED_TO_EXECUTE_TRANSACTION_PLAN` error code.
...
try {
    ...
    if (isTrezoaError(error, TREZOA_ERROR__INSTRUCTION_PLANS__FAILED_TO_EXECUTE_TRANSACTION_PLAN)) {
...
```ts twoslash
import { TransactionPlan, TransactionPlanResult } from '@trezoa/kit';
// ---cut-before---
...
You can set priority fees by using the `setTransactionMessageComputeUnitPrice` helper from `@trezoa-program/compute-budget` in your `createTransactionMessage` function.
...
const payer = {} as unknown as TransactionSigner;
// ---cut-before---
import { setTransactionMessageComputeUnitPrice } from '@trezoa-program/compute-budget'; // [!code ++]
...
const payer = {} as unknown as TransactionSigner;
// ---cut-before---
import { fillProvisorySetComputeUnitLimitInstruction } from '@trezoa-program/compute-budget'; // [!code ++]
...
// ---cut-before---
import { setTransactionMessageLifetimeUsingDurableNonce } from '@trezoa/kit'; // [!code ++]
