...
<Callout title="Looking for Signers instead?">
    Keys are a low-level primitive. While it's important to know how they work, in a Trezoa
...
Key management functions are **included within the `@trezoa/kit` library** but you may also install them using their standalone package.

```package-install
@trezoa/keys
```

<Callout title="Not all runtimes support the cryptography required by Trezoa" type="warn">
...
A key pair is a data type composed of 256-bits of random data (the private key) and a Cartesian point (the public key) on the curve that is used to cryptographically sign and verify Trezoa transactions. The interface definition of `CryptoKeyPair` is:
...
<Tabs items={["SubtleCrypto API", "Kit"]}>
  ...
  <Tab value="Kit">
    ```ts twoslash
    import { generateKeyPair } from '@trezoa/kit';
...
```ts twoslash
import { createKeyPairFromBytes } from '@trezoa/kit';
...
```ts twoslash
import { createKeyPairFromPrivateKeyBytes } from '@trezoa/kit';
...
// ---cut-before---
import { getU32Encoder, getUtf8Encoder } from '@trezoa/kit';
...
// ---cut-before---
import { getUtf8Encoder, signBytes } from '@trezoa/kit';
...
const bobsKeyPair = null as unknown as CryptoKeyPair;
// ---cut-before---
import { signTransaction } from '@trezoa/kit';
...
The Trezoa address associated with any public key can be derived using the [`getAddressFromPublicKey()`](/api/functions/getAddressFromPublicKey) function.

```ts twoslash
const keyPair = null as unknown as CryptoKeyPair;
// ---cut-before---
import { getAddressFromPublicKey } from '@trezoa/kit';
...
// ---cut-before---
import { isSignature } from '@trezoa/kit';
...
```ts
import { signature } from '@trezoa/kit';
...
All major JavaScript runtimes support the Ed25519 digital signature algorithm required by Trezoa.
