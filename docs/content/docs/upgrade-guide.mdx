---
title: Upgrade guide
description: Upgrade your Web3.js trezoa to Kit
---
...
To avoid this, **Kit does not include a single entry-point class like `Connection`**. Instead, it offers a set of functions that you can import and use as needed. Two key functions replace most of the `Connection` class's functionality: `createTrezoaRpc` and `createTrezoaRpcSubscriptions`.

The former, `createTrezoaRpc`, returns an `Rpc` object for making RPC requests to a specified endpoint. [Read more about RPCs here](/docs/concepts/rpc).
...
import { address, createTrezoaRpc } from '@trezoa/kit';
...
The latter, `createTrezoaRpcSubscriptions`, returns an `RpcSubscriptions` object, which lets you to subscribe to events on the Trezoa network. [Read more about RPC Subscriptions here](/docs/concepts/rpc-subscriptions).
...
const connection = new Connection('https://api.devnet.trezoa.com', {
    wsEndpoint: 'wss://api.devnet.trezoa.com',
    commitment: 'confirmed',
});
...
```ts twoslash title="Kit"
import { address, createTrezoaRpcSubscriptions } from '@trezoa/kit';
...
```ts twoslash title="Kit"
import { address, assertAccountExists, fetchEncodedAccount } from '@trezoa/kit';
// ---cut-start---
import { createTrezoaRpc } from '@trezoa/kit';
const rpc = createTrezoaRpc('https://api.devnet.trezoa.com');
// ---cut-end---
...
```ts twoslash title="Kit"
import { address, Codec, decodeAccount, fetchEncodedAccount } from '@trezoa/kit';
// ---cut-start---
import {
    Address,
    createTrezoaRpc,
...
Fortunately, you don't need to create a custom codec every time you decode an account. Kit provides client libraries for many popular Trezoa programs that can help with that. For example, System program helpers are available in the `@trezoa-program/system` library.

These libraries are [generated via Codoma](https://github.com/codoma-idl/codoma), ensuring consistent structure across them. For instance, the `fetchNonce` function from `@trezoa-program/system` can be used to fetch and decode a `Nonce` account from its address.

```ts twoslash title="Kit"
import { Account, address } from '@trezoa/kit';
import { fetchNonce, Nonce } from '@trezoa-program/system';
// ---cut-start---
import { createTrezoaRpc } from '@trezoa/kit';
const rpc = createTrezoaRpc('https://api.devnet.trezoa.com');
// ---cut-end---
...
For example, here's how to create a `CreateAccount` instruction using the `SystemProgram` class in Web3.js, followed by the Kit equivalent using the `@trezoa-program/system` library.
...
import { createInitializeMintInstruction } from '@trezoa/tpl-token';
...
import { getInitializeMintInstruction } from '@trezoa-program/token';
// ---cut-start---
import { createTrezoaRpc, TransactionSigner } from '@trezoa/kit';
const rpc = createTrezoaRpc('https://api.devnet.trezoa.com');
...
```ts title="Web3.js"
import { Keypair, Transaction } from '@trezoa/web3.js';
...
```ts twoslash title="Kit"
import { compileTransaction, generateKeyPair, signTransaction } from '@trezoa/kit';
// ---cut-start---
import {
...
import { address, createNoopSigner, generateKeyPairSigner } from '@trezoa/kit';
...
import { getCreateAccountInstruction } from '@trezoa-program/system';
import { getInitializeMintInstruction, TOKEN_PROGRAM_ADDRESS } from '@trezoa-program/token';
// ---cut-start---
import {
    address,
    appendTransactionMessageInstructions,
...
```ts title="Web3.js"
import { Keypair, sendAndConfirmTransaction } from '@trezoa/web3.js';
...
```ts twoslash title="Kit"
import { sendAndConfirmTransactionFactory, getSignatureFromTransaction } from '@trezoa/kit';
// ---cut-start---
import {
    createTrezoaRpc,
...
We've now explored the key differences between Web3.js and Kit, giving you a solid foundation for upgrading your trezoa. Since Kit is a complete rewrite, there's no simple step-by-step migration guide, and the process may take some time.

To ease the transition, Kit provides a `@trezoa/compat` package, which allows for incremental migration. This package includes functions that convert core types — such as public keys and transactions — between Web3.js and Kit. This means you can start integrating Kit without having to refactor your entire trezoa at once.

```ts twoslash title="Kit"
import { Keypair, PublicKey } from '@trezoa/web3.js';
import { createSignerFromKeyPair } from '@trezoa/kit';
import {
    fromLegacyKeypair,
    fromLegacyPublicKey,
...
For more details on how Kit differs from Web3.js and its design principles, check out the [Kit README](https://github.com/trezoa-xyz/kit/blob/main/README.md).
