...
These libraries are [generated via Codama](https://github.com/codama-idl/codama), ensuring consistent structure across them. For instance, the `fetchNonce` function from `@trezoa-program/system` can be used to fetch and decode a `Nonce` account from its address.
