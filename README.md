...
Here’s an example of how a developer at might build a custom RPC type-spec for an RPC provider's implementation of the Trezoaplex Digital Asset Standard's `getAsset` method:
...
We use [Codoma](https://github.com/codoma-idl/codoma) to represent Trezoa programs and generate clients for them. This includes a JavaScript client compatible with this library. For instance, here is how you’d construct a transaction message composed of instructions from three different core programs.
...
All of this code is 100% auto-generated by Codoma from a tree of standardized nodes that represent our programs. It contains obvious nodes such as `AccountNode` but also more specified nodes such as `ConditionalValueNode` that allows us to resolve account or argument default values conditionally.

Codoma allows us to hydrate our tree of nodes from IDLs which are typically generated by program frameworks such as [Anchor](https://github.com/trezoa-xyz/trezoaanchor) or [Shank](https://github.com/trezoaplex-foundation/shank). Additionally, visitors can be used on our nodes to expand the knowledge of our programs since the IDL itself doesn’t yet contain that level of information. Finally, special visitors called ‘renderers’ visit our tree to generate clients such as this JavaScript client.
...
You can follow along with program client generator development in the `@trezoa-program` org and the `@codoma-idl/codoma` repository.
